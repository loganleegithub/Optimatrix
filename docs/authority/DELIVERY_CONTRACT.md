...
persists only a minimal hit snapshot. A bounded full-stream capture is permitted only when the
task proves why that evidence is necessary; it is never the default response to uncertainty.
...
| Production-public Radar | real connectivity, coverage, natural anomaly, visible quote economics | fill, edge, Candidate quality |
...
- a public connectivity or natural-Radar-hit claim: production-public observation;
- a minimal hit: independent pure recomputation from its snapshot;
...
test and minimal snapshot can prove. Do not use synthetic fixtures to replace a required natural
market hit.

## Radar establishment evidence

The active [Short Vol Radar contract](../contracts/SHORT_VOL_RADAR.md) owns the detector, state,
denominator, minimal-hit snapshot, and establishment acceptance definitions. An implementation
task freezes that Policy before live evidence. Direct tests prove deterministic boundaries;
production-public observation proves only naturally occurring market facts.

The service runs until its declared business event occurs or a human stops it. Predetermined
elapsed time neither accepts nor rejects a capability and may not trigger threshold changes.
Evidence stays proportional to the claim: no full-feed archive, replay, Candidate, fill, Outcome,
or profitability proof is required to establish a Radar hit.
...
- `NO_HIT` requires usable detector inputs and known coverage.
- `ANOMALY_OBSERVED` requires a fired detector but no qualifying target-size atomic combo
  structure.
- `RADAR_HIT` requires both the anomaly and a qualifying atomic combo structure in the same strict
  as-of state.
- No hit creates no Shadow admission, executed entry, or Outcome object.
...
- distinct anomaly episodes after clear/re-arm;
- atomic quote-executable structures and non-qualifying legged references;
- distinct Radar-hit episodes;
...
Messages, detector recomputations, repeated quotes, legs, theoretical structures, elapsed runtime,
...
`NO_HIT` does not require a receipt. The minimal hit snapshot binds causal feature outputs and
enough structure facts to recompute the final trigger comparison and atomic economics, but not the
full rolling market input.

## Causality and Outcome

- Decision and hit inputs are known at or before their bound causal sequence.
...
- Does the Short Vol signal compare like-for-like remaining-life variance under a frozen Policy?
- Is an atomic quote-executable structure distinct from an anomaly, Candidate, Shadow admission,
  executed entry, and fill?
