...
activation. Its first authorized 43,200-second observation was rejected because the six-hour
session-local index tail never reached post-warmup scope: `RADAR_KNOWN / APPLICABLE_MARKET_SCOPE`
was `0 / 0` (`UNKNOWN`), with zero full-formula evaluations. A2 remains active at
`POST_WARMUP_APPLICABLE_SCOPE_NOT_REACHED`; the prior `NO_ACTIVE_COMBO` result remains a
market-structure diagnostic rather than a candidate-conversion claim. No persistent deployment or
further live command is authorized. The offline repair now bootstraps the economic baseline from
Deribit's official BTC-USDC index-chart history through one bounded in-memory validator, while the
streaming index remains responsible for current price/currentness. Exact five-minute history gaps
or staleness remain Radar `UNKNOWN`; they are never interpolated or repaired by reconnect logic.
