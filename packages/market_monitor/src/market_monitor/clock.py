...
class TrustedClock:
    ...
    def from_response(
        ...
        *,
        ...
        if stale_deadline_ms <= 0:
            raise ValueError("clock stale deadline must be positive")
        ...
        return cls(
            ...
            stale_deadline_ms=stale_deadline_ms,
        )
    ...
    def interval_at(self, monotonic_ms: int) -> TimeInterval:
        ...
        if monotonic_ms - self.last_refresh_monotonic_ms >= self.stale_deadline_ms:
    ...
    def refresh(
        ...
        fresh = TrustedClock.from_response(
            server_ms,
            sent_monotonic_ms,
            received_monotonic_ms,
            stale_deadline_ms=self.stale_deadline_ms,
        )
